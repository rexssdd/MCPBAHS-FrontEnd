...
export const Card = ({ children, style={}, className="", testId }) => (
  <div data-testid={testId} className={`card ${className}`} style={style}>{children}</div>
);
...
export const SectionTitle = ({ children, action, onAction }) => (
  <div className="section-title-row">
    ...
    {action && (
      <button className="section-action-btn" onClick={onAction}>{action} →</button>
    )}
  </div>
);
  ...
  return (
    ...
    >
      <span
        ...
        style={{ background:cfg.dot, animation:cfg.pulsing?"db-pulse 1.4s ease-in-out infinite":"none" }}
      />
      <span style={{ flex:1 }}><strong>{cfg.label}</strong></span>
...
/* ══════════════════════════════════════════════════════════
   STAT CARD
══════════════════════════════════════════════════════════ */
export const StatCard = ({ label, value, sub, subColor="#1a5c1a", tag, tagColor, loading }) => (
  <Card className="stat-card">
    <span className="stat-label">{label}</span>
    {loading
      ? <><Skel h={36} w="60%" r={8} mb={4}/><Skel h={12} w="80%"/></>
      : <>
          <span className="stat-value">{value}</span>
          {sub  && <span className="stat-sub" style={{ color:subColor }}>{sub}</span>}
          {tag  && <span className="stat-tag" style={{ color:tagColor, background:`${tagColor}14` }}>{tag}</span>}
        </>
    }
  </Card>
);
