...
const EMPTY_FILTERS = {
  ...
  schoolType:   "all",
  city:         "all",
  enrolledDate: "all",  // "all" | 0 | 7 | 30 | 90
};
...
function ApiStatusBanner({ status, onRetry }) {
  const CONFIG = {
    ...
    fallback: { dot: "fallback", text: "API not configured — displaying built-in sample data for UI testing. Set REACT_APP_API_URL to connect a backend." },
  };
...
/* ══════════════════════════════════════════════════════════════
   FILTER PANEL  ← NEW
   Slide-down panel attached to the "Filters" button.
   ══════════════════════════════════════════════════════════════ */
function FilterPanel({ filters, onChange, onReset, enrollees }) {
  /* Derive unique grade levels & cities that actually exist in the data */
  const availableGrades = [...new Set(
    enrollees.map(e => e.gradeLevel).filter(Boolean)
  )].sort();
  const availableCities = [...new Set(
    enrollees.map(e => e.city).filter(Boolean)
  )].sort();
      ...
      <div className="filter-panel__grid">
        ...
        <div className="filter-group">
          ...
          <div className="filter-chips">
            <button
              className={`filter-chip${filters.gradeLevel === "all" ? " filter-chip--active" : ""}`}
              onClick={() => onChange("gradeLevel", "all")}
            >All</button>
            {availableGrades.map(g => (
              <button
                key={g}
                className={`filter-chip${filters.gradeLevel === g ? " filter-chip--active" : ""}`}
        ...
        <div className="filter-group">
          ...
          <div className="filter-chips">
            <button
              className={`filter-chip${filters.schoolType === "all" ? " filter-chip--active" : ""}`}
              onClick={() => onChange("schoolType", "all")}
            >All</button>
            {SCHOOL_TYPES.map(t => (
              <button
                key={t}
                className={`filter-chip${filters.schoolType === t ? " filter-chip--active" : ""}`}
                onClick={() => onChange("schoolType", t)}
              >{t}</button>
            ))}
          </div>
        </div>
        ...
        <div className="filter-group">
          ...
          <div className="filter-chips">
            <button
              className={`filter-chip${filters.city === "all" ? " filter-chip--active" : ""}`}
              onClick={() => onChange("city", "all")}
            >All</button>
            {availableCities.map(c => (
              <button
                key={c}
                className={`filter-chip${filters.city === c ? " filter-chip--active" : ""}`}
        ...
        <div className="filter-group">
          ...
          <div className="filter-chips">
            <button
              className={`filter-chip${filters.enrolledDate === "all" ? " filter-chip--active" : ""}`}
              onClick={() => onChange("enrolledDate", "all")}
            >All time</button>
            {DATE_PRESETS.map(p => (
              <button
                key={p.label}
                className={`filter-chip${filters.enrolledDate === p.days ? " filter-chip--active" : ""}`}
                onClick={() => onChange("enrolledDate", p.days)}
              >{p.label}</button>
            ))}
          </div>
        </div>
        ...
        <div className="filter-tags">
          {filters.gradeLevel !== "all" && (
            <span className="filter-tag">
              {filters.gradeLevel}
              <button onClick={() => onChange("gradeLevel", "all")} aria-label="Remove grade filter">×</button>
            </span>
          )}
          {filters.schoolType !== "all" && (
            <span className="filter-tag">
              {filters.schoolType}
              <button onClick={() => onChange("schoolType", "all")} aria-label="Remove school type filter">×</button>
            </span>
          )}
          {filters.city !== "all" && (
            <span className="filter-tag">
              {filters.city}
              <button onClick={() => onChange("city", "all")} aria-label="Remove city filter">×</button>
            </span>
          )}
          {filters.enrolledDate !== "all" && (
            <span className="filter-tag">
              {DATE_PRESETS.find(p => p.days === filters.enrolledDate)?.label ?? "Custom date"}
              <button onClick={() => onChange("enrolledDate", "all")} aria-label="Remove date filter">×</button>
            </span>
          )}
        </div>
      )}
    ...
    <>
      ...
      <div className="form-card">
        ...
        <div>
          ...
          <div className="form-grid-3">
            <FormInput label="Email"         value={form.email} onChange={set("email")} placeholder="Email Address"/>
            <FormInput label="Phone number"  value={form.phone} onChange={set("phone")} placeholder="09XX XXX XXXX"/>
            <FormInput label="Date of Birth" value={form.dob}   onChange={set("dob")}   placeholder="mm/dd/yy"/>
          </div>
        </div>
...
function EnrolleeList({
  ...
  /* ── Apply tab + search + panel filters ── */
  ...
  /* ── Count per tab (always from full list, not filtered subset) ── */
    ...
    <>
      ...
      <div className="reg-tabs">
        {TABS.map(tab => (
          ...
          >
            {tab.label}
            {counts[tab.id] > 0 && (
              <span className="reg-tab__count">{counts[tab.id]}</span>
            )}
          </button>
        ))}
      </div>
      ...
      {(activeFilterCount > 0 || search) && !isLoading && (
        <div className="filter-results-bar">
          <span>
            Showing <strong>{filtered.length}</strong> of <strong>{byTab.length}</strong> enrollees
          </span>
          {(activeFilterCount > 0 || search) && (
            <button
              className="filter-clear-link"
              onClick={() => { handleFilterReset(); setSearch(""); }}
            >
              Clear all filters
            </button>
          )}
        ...
        <div className="empty-state">
          ...
          <p className="empty-state__subtitle">
            {activeFilterCount > 0 || search
              ...
              : "No enrollees have been added yet."}
          </p>
          {(activeFilterCount > 0 || search) && (
            <button
      ...
      <main id="main-content" className="page-main">
        <div className="page-body">
          ...
          {error && !isLoading && apiStatus === "error" && (
            <div style={{
              padding:"16px", background:"#FFEBEE", borderRadius:"8px",
              marginBottom:"20px", border:"1px solid #EF9A9A",
            }}>
              <p style={{ color:"#C62828", fontWeight:600, margin:"0 0 10px 0" }}>
                ⚠️ Error: {error}
              </p>
          ...
          {mutationError && (
            <div style={{
              ...
              <button className="btn btn-outline" onClick={dismissMutationError} style={{ fontSize:"13px" }}>
                Dismiss
              </button>
            </div>
          )}
